...
def test(k, x_test, y_test, BATCH_SIZE):
    PATH = '/opt/data/private/hello/seg/class/liver/attn/5fold/%d_fold_sgm_15-40x.pkl'%k
    testImgLoader = torch.utils.data.DataLoader(
        myImageFloderAttn(x_test, y_test, False), batch_size=BATCH_SIZE * 4, shuffle=True, num_workers=4, drop_last=False)
    models = mul_scale_attn(num_classes=2).cuda()
    models.load_state_dict(torch.load(PATH))
    models.eval().cuda()
    ...
    with torch.no_grad():
         ...
         plt.figure()
         plt.plot(fpr, tpr, color='darkorange', lw=2, label='Mul_Scale_Attn_20x(AUC={:4f})'.format(auc_value))
         ...
         plt.savefig('/opt/data/private/hello/seg/class/liver/attn/5fold/%d_fold_sgm_15-40x.png'%k)



    return acc_sorce, auc_value, fpr, tpr




def k_fold_test(bp=0, k=5, batch_size=16):

    test_acc_all, test_auc_all, test_spe_all, test_sen_all = [], [], [], []
    for i in range(bp, k):
        print('*' * 25, '第', i + 1, '折', '*' * 25)
        # 每份数据进行训练
        xt, yt = dataloader_k_fold(k, i, True)
        test_acc, test_auc, test_spe, test_sen = test(i, xt, yt, batch_size)
        print('test_acc is ', test_acc)
        print('test_auc is ', test_auc)
        print('test_spe is ', test_spe)
        print('test_sen is ', test_sen)
        test_acc_all.append(test_acc)
        test_auc_all.append(test_auc)
        test_spe_all.append(test_spe[1])
        test_sen_all.append(test_sen[1])

    test_acc_all, test_auc_all, test_spe_all, test_sen_all = np.array(test_acc_all), np.array(test_auc_all), np.array(test_spe_all), np.array(test_sen_all)
    print('mean test_acc is ', np.mean(test_acc_all), np.std(test_acc_all))
    print('mean test_auc is ', np.mean(test_auc_all), np.std(test_auc_all))
    print('mean test_spe is ', np.mean(1-test_spe_all), np.std(1-test_spe_all))
    print('mean test_sen is ', np.mean(test_sen_all), np.std(test_sen_all))
